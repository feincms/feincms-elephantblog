...
class Command(NoArgsCommand):
    ...
    def handle_noargs(self, **options):
            ...
            if self.use_sites:
                for site in entry.sites.all():
                    create_kwargs = {
                        ...
                        "changesurl": site.domain
                        + ":/"
                        + entry.get_absolute_url(),  # noqa
                    }
        ...
        PingedURL.objects.filter(
            created=timezone.now() - timedelta(days=7), status=PingedURL.SUCCESSFUL,
        ).delete()
        ...
        Entry.objects.filter(
            id__in=pingedurl_queryset.filter(status=PingedURL.SUCCESSFUL).values(
                "object_id"
            )
